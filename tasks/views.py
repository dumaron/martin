...
from .models import Inbox, Project, Todo
...
def project_detail(request, project_id):
	...
	return render(
		request,
		'project.html',
		{'project': project, 'todos': related_todos, 'children_projects': children_projects},
	)
...
def process_inboxes_flow(request):
	...
	oldest_unprocessed_inbox = (
		Inbox.objects.filter(processed_at__isnull=True, deleted_at__isnull=True).order_by('created_at').first()
	)
