...
# Substring patterns (lowercase) used to identify food categories and map them to a canonical name.
# Handles emoji-prefixed names ('🛒 Groceries') and capitalisation variants ('Eat out').
FOOD_PATTERNS = [('grocer', 'Groceries'), ('eat', 'Eat Out'), ('delivery', 'Delivery')]
...
def build_monthly_expense_report(year, month) -> MonthlyReport:
	...
	"""
	history = _months_back(year, month, HISTORY_MONTHS)
...
def list_available_months():
	"""
	Distinct (year, month) tuples on the shared bank account that have at least one paired
	non-transfer expense, newest first. Used to populate the report list page.
	"""
	rows = (
		BankTransaction.objects.filter(
			bank_account__personal=False,
	...
	return [(d.year, d.month) for d in rows]
